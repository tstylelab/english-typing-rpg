...
try {
  ...
  await page.getByRole('button', { name: /^英検3級/ }).click();
  ...
  await page.setViewportSize({ width: 1366, height: 900 });
  await page.screenshot({ path: 'node_modules/.tmp/grade3-result.png' });
  ...
  assert.equal(saved.difficulty, 'Eiken3');
  assert.equal(saved.level, 3);
  await page.getByRole('button', { name: /英語タイピングバトル/ }).click();
  assert.ok(await page.locator('option[value="Eiken3"]').count());
  await page.getByLabel('プレイヤー1の教材', { exact: true }).selectOption('Eiken3');
  ...
  for (const level of [1, 2, 3]) {
    for (const [mode, inputMode] of [['guide', 'voice-text'], ['challenge', 'voice-text'], ['challenge', 'voice-only'], ['challenge', 'text-only']]) {
      await page.evaluate(({ level, mode, inputMode }) => {
        localStorage.setItem('etyping_last_selected_course', JSON.stringify({ difficulty: 'Eiken3', level, resumeMode: mode, resumeInputMode: inputMode }));
  ...
  console.log('PASS Grade 3 selection, sentence battle, grammar result, responsive layout, reload, versus start and all 12 level/mode routes');
