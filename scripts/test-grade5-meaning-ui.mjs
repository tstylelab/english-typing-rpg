...
const corrections = JSON.parse(readFileSync(new URL('../src/data/grade5MeaningCorrections.json', import.meta.url), 'utf8'));
...
try {
  ...
  await page.addInitScript(rows => {
    ...
    if (!localStorage.getItem('meaning-test-seeded')) {
      localStorage.setItem('etyping_manual_question_statuses', JSON.stringify(Object.fromEntries(rows.map(q => [
        `Eiken5:${q.level}:${q.text}:${q.translation}`,
  ...
  for (const width of [1366, 390]) {
    ...
    {
      await page.getByRole('button', { name: '英検5級', exact: true }).click();
      for (const level of [1, 2, 3]) {
        ...
        console.log(`PASS: Grade 5, Level ${level}, ${group.length} corrections, ${width}px, saved state retained`);
      }
    }
  }
