...
const corrections = JSON.parse(readFileSync(new URL('../src/data/pre1SentenceMeaningCorrections.json', import.meta.url), 'utf8'));
...
try {
  ...
  await page.addInitScript(rows => {
    ...
    if (!localStorage.getItem('meaning-test-seeded')) {
      localStorage.setItem('etyping_manual_question_statuses', JSON.stringify(Object.fromEntries(rows.map(q => [
        `EikenPre1Part${q.part}:${q.level}:${q.text}:${q.translation}`,
  ...
  for (const width of [1366, 390]) {
    ...
    for (const part of [1, 2]) {
      await page.getByRole('button', { name: `英検準1級${part === 1 ? '①' : '②'}`, exact: true }).click();
      for (const level of [2, 3]) {
        await page.getByRole('button', { name: `Level ${level}`, exact: true }).click();
        const group = corrections.filter(q => q.part === part && q.level === level);
        ...
        console.log(`PASS: Part ${part}, Level ${level}, ${group.length} corrections, ${width}px, saved state retained`);
      }
    }
  }
