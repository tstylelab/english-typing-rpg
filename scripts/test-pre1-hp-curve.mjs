...
for (const input of ['voice-only','text-only']) {
  expected.forEach((hp, i) => {
    const result = ctx.tune('EikenPre1Part1',1,'challenge',input,i,previous[i],0);
    assert.equal(result.monsterHp,hp);
    assert.equal(result.maxQuestions,10);
    for (const difficulty of ['Eiken5','Eiken4','EikenPre1Part2']) {
    ...
    for (const level of [2,3]) assert.equal(ctx.tune('EikenPre1Part1',level,'challenge',input,i,2000,0).monsterHp,2700);
  });
  [3618,5427,7236,9045].forEach((hp,i) => assert.equal(ctx.tune('EikenPre1Part1',1,'challenge',input,19+i,1340,i+1).monsterHp,hp));
}
for (const mode of ['guide','challenge']) {
  expected.forEach((_,i) => assert.equal(ctx.tune('EikenPre1Part1',1,mode,'voice-text',i,230,0).monsterHp,311));
}
