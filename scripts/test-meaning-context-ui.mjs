...
const corrections = [
  ...JSON.parse(readFileSync(new URL('../src/data/grade4MeaningCorrections.json', import.meta.url), 'utf8')).map(q => ({ ...q, course: 'Eiken4' })),
  ...JSON.parse(readFileSync(new URL('../src/data/intermediateMeaningCorrections.json', import.meta.url), 'utf8')),
];
const courses = { Eiken4: '英検4級', Eiken3: '英検3級', EikenPre2: '英検準2級', Eiken2: '英検2級' };
...
try {
  ...
  await page.addInitScript(rows => {
    ...
    if (!localStorage.getItem('meaning-test-seeded')) {
      localStorage.setItem('etyping_manual_question_statuses', JSON.stringify(Object.fromEntries(rows.map(q => [
        `${q.course}:${q.level}:${q.text}:${q.translation}`,
  ...
  for (const width of [1366, 390]) {
    ...
    for (const [course, label] of Object.entries(courses)) {
      await page.getByRole('button', { name: label, exact: true }).click();
      for (const level of [1, 2, 3]) {
        await page.getByRole('button', { name: `Level ${level}`, exact: true }).click();
        const group = corrections.filter(q => q.course === course && q.level === level);
        ...
        console.log(`PASS: ${label}, Level ${level}, ${group.length} corrections, ${width}px, saved state retained`);
      }
    }
  }
  assert.deepEqual(errors, []);
  console.log('PASS: all course/viewport checks completed, no browser errors.');
  await page.close();
