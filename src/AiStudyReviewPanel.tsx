...
type Props = { recorder: AiStudyRecorder; courseLabels: Record<string, string> };
...
export default function AiStudyReviewPanel(props: Props) {
  ...
  return <section className="mb-4 flex-shrink-0 rounded-xl border border-violet-400/30 bg-violet-950/15 p-3">
    ...
    {open && createPortal(<ReviewDialog {...props} onClose={() => setOpen(false)} />, document.body)}
  </section>;
}

function ReviewDialog({ recorder, courseLabels, onClose }: Props & { onClose: () => void }) {
  ...
  const copy = async () => {
    ...
    try {
      ...
      setStatus('コピーしました。お使いのAIに貼り付けてください。');
    } catch {
      setPreview(true);
  ...
  return <dialog ref={dialog} onClose={onClose} aria-labelledby="ai-review-title" className="fixed inset-0 m-auto max-h-[85dvh] w-[min(36rem,calc(100%-2rem))] overflow-y-auto overscroll-contain rounded-xl border border-violet-400/40 bg-slate-900 p-4 text-sm text-slate-200 shadow-xl backdrop:bg-black/60">
    <div className="mb-3 flex items-center justify-between gap-3">
      <h2 id="ai-review-title" className="font-bold text-violet-100">AIに学習相談</h2>
    ...
    <div className="space-y-3">
      ...
      <label className="flex items-center gap-2">対象期間
        <select aria-label="AI相談の対象期間" disabled={copying} value={period} onChange={e => { setPeriod(e.target.value as ReviewPeriod); setReport(''); setPreview(false); setStatus(''); }} className="rounded-lg border border-slate-600 bg-slate-950 px-3 py-2">
      ...
      <details className="text-xs leading-5 text-slate-400">
        ...
        <button type="button" disabled={copying} className="mt-2 py-2 underline" onClick={() => {
          ...
          recorder.clear(); setCount(0); setReport(''); setPreview(false); setStatus(recorder.warning ? 'この画面の記録は消去しましたが、保存先を更新できませんでした。' : '相談用の記録を削除しました。');
        }}>相談用の記録を削除</button>
      </details>
    </div>
  </dialog>;
