// Grade 3 alone: keep word battles at the Grade 4 baseline, while shortening
// phrase/sentence battles. HP is based on attainable damage, not vocabulary count.
export const getGrade3QuestionLimit = (level: 1 | 2 | 3, bossStage: number) => (
  level === 1 ? [10, 20, 30, 40, 50] : level === 2 ? [10, 16, 20, 24, 28] : [8, 12, 16, 20, 24]
)[bossStage] ?? 10;

export const getGrade3BossHpMultiplier = (level: 1 | 2 | 3, bossStage: number) => (
  level === 1 ? [1, 2, 3, 4, 5] : [1, 1.5, 2.3, 3.1, 4]
)[bossStage] ?? 1;

export const getGrade3BaseHp = (level: 1 | 2 | 3, learning: boolean, stageIndex: number, fallback: number) => {
...
export const getGrade3MissMultiplier = (level: 1 | 2 | 3, misses: number, length: number) => {
