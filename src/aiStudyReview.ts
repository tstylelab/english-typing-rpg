...
export function buildAiStudyReport(records: StudyRecord[], period: ReviewPeriod, courseLabels: Record<string, string>, now = Date.now(), battleRecords?: StudyRecord[]): string {
  ...
  const lines = [
    ...
    '【2. 記憶に残すためのTips】',
    '再ミスの多い3〜5語に絞り、各語は「単語＋覚えるきっかけの短い見出し」と本文2〜3文。目安80〜160文字ですが、伝われば短くて構いません。候補が3語未満ならその語だけ。',
    '各Tipsは「覚えるきっかけ」→「その文字を思い出せる短い理由」の順で。ミスの位置・回数は表に任せ、Tipsで繰り返さないでください。裏では実際のミスに合わせ、本文はイメージ・語呂・身近な関連語から始めてください。',
    ...
    '「正しく文字を並べれば、その位置は正解になる」という正解の言い直しは禁止。「ひとかたまりで覚える」「標識を想像する」「繰り返す」だけでも終わらず、実際の誤入力と正解の違いに結び付けてください。',
    '【文体の完成例：学習データではない】',
    '以下は書き方だけの見本です。例の単語やミスを候補へ追加せず、候補にあっても実際のミスと合う場合だけ参考にしてください。',
    'temporary：RでAをサンドイッチ！\ntempo【RAR】yと見ると、Rが両側からAをはさんでいます。「Aを守るRが2つ」と覚えておくのもよさそうです♪ Lと迷ったときの目印になりますね。※スペル暗記用のイメージです。',
    'barrier：Rが2枚の壁に\n「壁・障害」のbarrierは、ba【rr】ierとRが2つ並びます。Rを壁の板2枚に見立てると、1枚忘れずにすみそうですね。※スペル暗記用のイメージです。',
    'replay：もう一回、play！\nゲームでも「リプレイ」って見かけますよね。replayのre-は「もう一度」で、playはそのまま残ります。re＋playと組み立てると、途中のeを思い出す目印になります。',
    '【3. 共通するミスへの短いヒント（根拠がある場合のみ）】',
    '文字の取り違え集計から最大2項目、各2文程度。記録されたL/Rやa/eなどについて、実例に結び付けた綴りの覚え方を添えてください。これらの組合せが記録になければ作らないでください。',
    'ここもヒントから始め、位置数・語数の報告で書き出さないでください。1語だけなら「この語では」と限定し、複数語で観測された傾向と区別。表とTipsの繰り返しや一般的な発音指導で埋めないでください。',
    '前置き・総評・長い傾向分析・練習メニュー・締めの言葉は不要。',
    '回答前に内部で確認：ミスに合った手掛かりから始まるか、綴り・区切り・文字位置が正しいか、同じ綴りを「AではなくA」と比較していないか。矛盾や正解の羅列は書き直し、確認作業自体は出力しないでください。',
    '【注意（回答には繰り返さない）】',
