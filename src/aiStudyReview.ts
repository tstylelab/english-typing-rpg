...
export function buildAiStudyReport(records: StudyRecord[], period: ReviewPeriod, courseLabels: Record<string, string>, now = Date.now(), battleRecords?: StudyRecord[]): string {
  ...
  const lines = [
    ...
    '小学生にも話しかける、親しみのある自然な話し言葉で。「〜してみよう」「〜もアリだよ」くらいの調子に。分析レポートの語尾だけを砕くのではなく、思い出す手掛かりを一緒に見つける文章にしてください。',
    ...
    '実際の語の構造と暗記用の区切りは区別し、見た目だけの工夫には「暗記用のイメージだよ」など短い注意を添えてください。無理な語源説明は不要です。',
    ...
    'temporary：AをRではさんじゃおう。\nR・A・Rを「Rが両側からAを守ってる」と見てみよう。tempo【RAR】yと目立たせれば、Lと迷ったときの目印になるよ。※暗記用のイメージだよ。',
    'barrier：Rが2枚の壁になった！\n「壁・障害」のbarrierは、ba【rr】ierとRが2つ並ぶよ。Rを壁の板2枚に見立てたら、1枚忘れずにすみそうだね。※暗記用のイメージだよ。',
    'replay：もう一回、play！\nゲームでも「リプレイ」って見るよね。replayのre-は「もう一度」で、playはそのまま残るよ。re＋playと組み立てれば、途中のeを思い出す目印になるね。',
