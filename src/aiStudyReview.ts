...
export function buildAiStudyReport(records: StudyRecord[], period: ReviewPeriod, courseLabels: Record<string, string>, now = Date.now(), battleRecords?: StudyRecord[]): string {
  ...
  const lines = [
    ...
    '【2. 記憶に残すためのTips】',
    '再ミスのある語を優先し、記録から具体的に言える語を最大5語。各語は短い箇条書きで、英語を太字にして1〜2文・上限の目安160文字。最低文字数・最低件数は設けず、見出しや空行で縦幅を増やさないでください。',
    '実際に間違えた箇所や取り違えを一言だけ伝えてください。記録された「正解→実際の入力」の方向を守り、繰り返しがあればその箇所を短く強調。1回だけなら「今回の記録では」と限定し、頻繁な苦手と決めつけないでください。詳細がなければ位置や文字を作らず、その語のミス記録があることだけ伝えて構いません。',
    '役立つ覚え方が自然に見つかる場合だけ、ミスした箇所に結び付く思い出す手掛かりを1つ添えてください。なければミス箇所の短い指摘だけで終了して構いません。覚え方を無理にひねり出したり、件数や文字数を埋めるために文章を足さないでください。',
    ...
    '「今回も以前もミスあり」の候補があればTipsで優先。ただし今回だけのミスを長年の苦手と決めつけず、以前の履歴の語も取り上げてください。',
    ...
    '覚え方として「正しく並べれば正解になる」という正解の言い直しは禁止。「ひとかたまりで覚える」「繰り返す」だけの一般論を足すくらいなら、記録の指摘だけにしてください。',
    '【文体の完成例：学習データではない】',
    '以下は書き方だけの見本です。例の単語やミスを候補へ追加せず、候補にあっても実際のミスと合う場合だけ参考にしてください。',
    '指摘だけの例（5文字目u→tが別々の出題で2回記録された場合）：**executive**：5文字目のuをtにするミスが、2回の出題で出ています。今回はこのuが確認ポイントですね。',
    '覚え方を添える例（後半r→lの記録がある場合）：**temporary**：後半のrをlにするミスが出ています。tempo【RAR】yは「RでAをサンドイッチ！」と覚えておくのもよさそうです♪ ※スペル暗記用のイメージです。',
    '【3. 共通するミスへの短いヒント（根拠がある場合のみ）】',
    '複数語に共通する記録があり、個別Tipsと重複しない役立つヒントがある場合だけ最大2項目・各1〜2文。なければこの見出しごと省略してください。記録にないL/Rやa/eなどを作らず、一般論や無理な覚え方で埋めないでください。',
    '前置き・総評・長い傾向分析・練習メニュー・締めの言葉は不要。',
    '回答前に内部で確認：表は英語と和訳・類義語だけか、Tipsの指摘は記録と一致するか、覚え方を無理に足していないか、綴り・区切り・文字位置が正しいか、同じ綴りを「AではなくA」と比較していないか。矛盾は書き直し、確認作業自体は出力しないでください。',
    '【注意（回答には繰り返さない）】',
