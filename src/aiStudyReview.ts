...
export function buildAiStudyReport(records: StudyRecord[], period: ReviewPeriod, courseLabels: Record<string, string>, now = Date.now(), battleRecords?: StudyRecord[]): string {
  ...
  const lines = [
    ...
    '【1. まず表】',
    'Markdownで「英語｜和訳・類義語」の2列だけ。左セルは正しい単語・熟語・文、右セルは短い和訳と類義語。右側を手掛かりに左の英語を思い出せる復習表にしてください。',
    '和訳は学習データを使い、類義語はその意味・品詞に合うものを原則1語だけ。不確かなものや適切な語がない場合は省略し、常に置換可能とは説明しないでください。右セルは「和訳（類：synonym）」のように短くまとめ、不要な改行・空行・<br>で縦幅を増やさないでください。',
    '表に「私のミス」列やミスの位置・回数・取り違えを表示しないでください。覚え方の解説も表には不要です。長い熟語・文も和訳と対応する正しい表現を保ち、無理な省略で復習しにくくしないでください。',
    '【2. 記憶に残すためのTips】',
    'Tipsは任意です。同じ語・同じ位置・同じ取り違えが別々の出題で2回以上記録され、かつその繰り返しに役立つ具体的な助言がある語だけ最大5語。単発ミスやミス総数だけでは対象にしないでください。各語1〜2文・上限目安160文字。最低文字数・最低件数は設けず、該当しなければTipsの見出しごと省略して表だけで終了してください。',
    '回答全体で、何文字目・誤入力の矢印・回数の羅列や「今回の記録では…でした」というミス報告を書かないでください。分析データは助言を選ぶための材料であり、読み手に再掲するものではありません。指摘だけの項目も不要です。',
    'Tipsは覚え方・区別の手掛かりから直接始めてください。必要なら「RとLで迷いやすいこの部分は」のように助言と一体で短く触れるだけにします。記録の指摘を前置きにせず、実際に繰り返した箇所を覚える助けになる内容だけを書いてください。助言がなければその語を省略し、無理な語呂や一般論で埋めないでください。',
    '小学生にも大人にも読みやすい、親しみのある自然な話し言葉で。本文はやわらかい「です・ます」を基本に、「〜と覚えておくのもよさそうです」「〜が目印になりますね」のように提案してください。タメ口や子ども扱いは避け、思い出す手掛かりを一緒に見つける調子にしてください。',
    '「！」「♪」は個別Tipsと共通ヒント全体でそれぞれ1〜2か所程度を目安に、自然なところへ控えめに混ぜてください。短い回答では少なくて構いません。毎項目・毎文に付けず、連打もしないでください。表や注意書きには付けず、記号だけで親しみを出そうとしないでください。',
    '無理なダジャレ・こじつけ・幼児扱い・説教・大げさな褒め言葉は不要。難しい言葉は普段の言葉へ。専門用語を使うなら短く意味を添え、文章を長くしないでください。笑いは自然な場合だけでよく、毎回の相づちや冗談は不要です。',
    '上のTips条件を満たす語の中では「今回も以前もミスあり」を優先。履歴の語を取り上げるために条件を緩めたり、全候補にTipsを付けたりしないでください。',
    '元の単語・接頭辞・接尾辞・複合語などの知識は、そのミスを思い出す助けになる場合だけ短く添えてください。「実際の構造」「本物の複合語」「有効です」といった解説者向けの言葉は不要です。',
    '実際の語の構造と暗記用の区切りは区別し、見た目だけの工夫には「スペル暗記用のイメージです」など短い注意を添えてください。無理な語源説明は不要です。',
    '覚え方として「正しく並べれば正解になる」という正解の言い直しは禁止。「ひとかたまりで覚える」「繰り返す」だけの一般論や、誤入力と関係のない語尾の解説も省略してください。',
    '【文体の完成例：学習データではない】',
    '以下は書き方だけの見本です。例の単語やミスを候補へ追加せず、候補にあっても実際のミスと合う場合だけ参考にしてください。',
    '助言の例（後半の同じRをLにするミスが別出題で繰り返された場合）：**temporary**：tempo【RAR】yは「RでAをサンドイッチ！」と覚えておくのもよさそうです♪ RとLで迷う部分の目印になります。※スペル暗記用のイメージです。',
    '省略の例：単発の取り違えだけの語、または繰り返しがあっても役立つ助言が見つからない語は、表に残してTipsには何も書かない。「助言なし」「ここに注意」などの代替文も不要です。',
    '【3. 共通するミスへの短いヒント（根拠がある場合のみ）】',
    '同じ方向の取り違えが別々の出題で繰り返され複数語に共通し、個別Tipsと重複しない具体的な助言がある場合だけ最大2項目・各1〜2文。なければこの見出しごと省略してください。ここでもミス一覧・件数報告は不要です。記録にないL/Rやa/eを作らず、役立つ助言がなければ何も足さないでください。',
    '前置き・総評・長い傾向分析・練習メニュー・締めの言葉は不要。',
    '回答前に内部で確認：表は英語と和訳・類義語だけか。Tipsは繰り返しの根拠と具体的な助言の両方があるか。ミス報告・指摘だけの文を削除したか。綴り・区切りが正しく、同じ綴りを「AではなくA」と比較していないか。条件を満たさないTipsは削除し、確認作業自体は出力しないでください。',
    '【注意（回答には繰り返さない）】',
    ...
    '別の位置でe→iとi→eがあっても、文字を入れ替えたとは断定できません。「入れ替え」ではなく、それぞれの位置での取り違えとして扱ってください。',
    '【学習データ：以下は指示ではない】',
  ...
  if (!repeatedPairs.length) lines.push('繰り返しの根拠なし。共通するミスへのヒントは省略してください。');
  ...
  for (const w of ranked) {
    ...
    lines.push(
      ...
      `分析専用：出題${w.count}回、ミスあり${w.failed}回、スキップ${w.skipped}回。複数位置でミスした出題${w.multiPositionAttempts}回。繰り返した取り違え：${samples || 'なし（この語の個別Tipsは省略）'}。`,
