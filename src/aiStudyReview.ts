...
export function buildAiStudyReport(records: StudyRecord[], period: ReviewPeriod, courseLabels: Record<string, string>, now = Date.now(), battleRecords?: StudyRecord[]): string {
  ...
  const lines = [
    ...
    '【3. まとめ：次の単語にも使える覚え方】',
    '最後に、今回の候補を具体例に、ほかの単語にも応用できる覚え方をまとめてください。パーツの意味、知っている言葉とのつながり、綴りを見分ける目印など、今回の語に合う着眼点を選び、「なぜ役立つか」「次にどう使えるか」を説明してください。共通点は無理に作らず、個別Tipsの丸写しや一般的な励ましだけにしないでください。',
    'まとめには個別Tipsの1〜2文制限を適用せず、必要なら詳しくして構いません。短い見出しと箇条書きで、最初の一文だけでも要点が分かる形に。反復ミスの記録があれば役立つ着眼点に生かし、なければ単語そのものから説明してください。ミス一覧や能力の断定は不要です。最後は今回の例を使った、気軽に試せる一つの工夫で締めてください。',
